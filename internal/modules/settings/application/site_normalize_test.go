...
func TestNormalizeStorefrontTemplate(t *testing.T) {
	tests := []struct {
		...
		{name: "editorial", raw: "editorial", want: constants.StorefrontTemplateEditorial},
		{name: "catalog", raw: "catalog", want: constants.StorefrontTemplateCatalog},
		{name: "spotlight", raw: "spotlight", want: constants.StorefrontTemplateSpotlight},
...
func TestUpdateSiteSettingNormalized(t *testing.T) {
	...
	if contact["wechat"] != "cardpay-wechat" {
		t.Fatalf("unexpected wechat: %v", contact["wechat"])
	}
	if contact["qq"] != "12345678" {
	...
	if controls["show_banner"] != false ||
		controls["show_language_switcher"] != false ||
